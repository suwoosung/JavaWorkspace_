...
public class C_Scanner {
	...
	// 사용자가 키보드로 직접 입력한 값으 변수에 저장할 수 있게 도와주는 클래스
	...
	public void inputTest1() {
		...
		// 2. next() : 사용자가 입력한 값을 공백으로 가져오는 메서드.
		
		//String name = sc.nextLine();
		
		String firstname =sc.next();// 서\n
		sc.nextLine();// 개행문자 비워주기 작업 필요
		String lastname = sc.nextLine();// 우성
		...
		//입력한 데이터를 문자혀응로 뽑기
		// String의 CharAt(0) => 문자열의 첫번쨰 글자를 문자로 뽑는 메서드
		System.out.println("당신의 성별은?(M/F)");
		//char gender = sc.next().charAt(0);
		String gender = sc.next();
		char gneder2 = gender.charAt(0);
	}
}
