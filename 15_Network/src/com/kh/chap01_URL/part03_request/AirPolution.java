...
public class AirPolution {
	...
	public static void main(String[] args) {
		 ...
		 *    - API 서버 
		 *    - URL로 데이터에 대한 요청을 보내면 응답데이터를 반환해주는 서버로 애플리케이션들 중 데이터나 서비스를 제공하는
		 *    역할을 API서버라고 부든다(카카오 API서버,네이버 API서버 등) 
		 *    - A애플리케이션이 B애플리케이션에 필요한 데이터가 있는 경우, B애플리케이션의 API서버를 통해 요청(request)를 보내고, B API서버는 요청에
		 *    응답(response)을 돌려준다. 
		 ...
		 *  
		 *    1) Get방식 - URL에 요청변수(quertParameter)를 붙여서 요청하는 방식 - 요청변수는 key=value형태로
		 *    작성하며 여러개의 요청변수가 있을 경우 &로 구분한다. 
		 ...
		 * 
		 *   2) JSON유형의
		 *     데이터(표준) { "name" : "mkm" } // 문자열 형태의 javascript 객체 
		 *     
		 *   3) 처리방법 - xml , json등 해당
		 *     타입에 맞는 파서를 이용하여 내 프로그래밍언어에 맞는 객체로 변환한다.
		 * 
		 * 
		 * 4. API 응답 상태코드 
		 *  - 200 : 성공
