...
public class ListRun {
	/*
	 * 컬렉션(Collection)
	 *  - 자료구조가 내장되러있는 클래스로, 자바에서 제공하는 자료구조를 담당하는 프레임워크
	 *  
	 * 자료구조
	 ...
	 *  2. 중간에 값을 추가하거나, 삭제하는등 배열내부의 값들을 조작하기위한 편리한 메서드가 정의 되거 있다.
	 ...
	 *  
	 * Map계열
	 *  - 담고자 하느 값(value)과 그 값을 저장하는 키(key)를 함께 저장
	 *  - 값 저장시 순서를 유지하지 않는다(index x)
	 *  - key값의 중복을 허용하지 않으나, value의 중복은 허용
	 *  - HashMap, Properties,...
	 */
	...
	public static void main(String[] args) {
		...
		ArrayList<Music> list = new ArrayList<>(3); // 객체생성시에는 제네릭 생성가능.
		...
		//  - index1에서부터 index2까지 데티러를 추출한 새로운 subList를 반환하는 메서드
		...
		// 리스트에 컬렉션에 잇는 데이터를 모두 추가해주는 메서드.
		list.addAll(list2);
		 ...
		 *  - vo클래스 이외 병도 클래스에 Comparator를 상속시켜서 구현한다.
		 *  - 여러개의 정렬조건을 만들 수 있다.
		 */
		System.out.println("==============================================");
