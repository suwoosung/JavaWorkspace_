package com.kh.chap04_map.part01_hasMap.run;
...
public class MapRun {
	...
	public static void main(String[] args) {
		 ...
		 *    가 저장된 위치에 접근한다.
		 *  - key값의 중복을 허용하지 않는다.
		 *  - 동일한 key값에 데이터를 저장하며느 마지감으로 추가된 value값이 저장된다.
		 */
		// key값의 자료형은 String
		// value값의 자료형은 Snack
		...
		//  - hashmap에서 key값과 일치하는 값을 찾아, 새로 전달괸 value로
		//    "교체"하는 메서드
		hm.replace("다이제", new Snack("고소한맛", 500));
		
		// 5. remove(Key K) : KEY값과 일치하는 값을 찾아서 제서해주는 메서드
		//					  key와 value 함계 제거된다.
		hm.remove("다이제");
		...
		// - keySet() : 맵의 key값들을 따로 Set에 담아 반환애주는 메서드
		...
		//  - 맵의 key,value값이 감신 entry을 set에 담아 반환해주는 메서드.
