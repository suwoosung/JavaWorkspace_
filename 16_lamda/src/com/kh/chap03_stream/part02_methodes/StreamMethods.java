package com.kh.chap03_stream.part02_methodes;
...
public class StreamMethods {
	public static void main(String[] args) {
		 ...
		 *    -> 스트림 내부 요소을 내가 원하는 계열의 컬렉션으로 반환
		 * 3) Calculating 계열 : min, max, sum, average 등
		 *    -> 산술연산관련된 최종 처리 메서드들(기본자료형에만 존재)
		 * 4) Matching : anyMatch, allMatch, noneMatch...
		 *    -> 스트림 내부 요소에서 특정 조건을 만족하는 요소을 선택하기 위해
		 *       사용
		...
		// 3) mapTo(Int/Double/Long) : Stream을 기본자료형으로 변경해주는 메서드.
		Arrays
			.stream(names)
		...
		// 4) Collect계열 메서드
		// 4-1) Collectors.toList : 객체로 최종 변환해주는 메서드
		...
		List<Integer> list4 = Arrays.asList(1,1,2,2,2,3,3,4,4,5,5,6);
		Set set = list4.stream()
				.collect(Collectors.toSet());
		System.out.println(set);
		
		// 4-3) Collectors.toMap(Function, Function) : t트림의 결과를 Map으로
		...
		// 6) reduction계열 : 여러 스트림 요소들을 하나로 합치는 연산
