...
public class MethodReference {
	 ...
	 *  - 메서드참조는 람다식이 수행하는 일이 "어떤 메서드를 그래도 호출하는 것"일 때
	 *    그 메서드의 이름만으로 람다식을 대체하는 문법
	 *  - 기존 람다식 구문을 더욱 간결하게 표현하기 위하여 사용한다.
	 *    ex) (str) -> {return str.length();}
	 *        String::length;
	 *  - 람다식을 통해 구현한 기능이 이미 정의되어있는 다름 메서드의 기능을 호출하고,
	 *    반환시키거나 출력하거나 단순한 역할을 수행할 떄, 드 다른메서드ㅡ이 이름만 추가시키는
	 *    형태로 작성하는 구문.
	 *    
	 * 메서드 참조의 종류
	 * 1. static 메서드 참조
