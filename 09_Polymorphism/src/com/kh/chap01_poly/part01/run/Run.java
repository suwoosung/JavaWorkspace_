...
import com.kh.chap01_poly.part01.movel.vo.Child1;
import com.kh.chap01_poly.part01.movel.vo.Child2;
import com.kh.chap01_poly.part01.movel.vo.Parent;
...
public class Run {
	...
	public static void main(String[] args) {
		...
		p2.printParent();// p2(부모타입변수)또는 Parent클래스내부 메서드/필드만 접근가능.
		// 자식클래스의 메서드에 접근하고자 안다면, DownCasting이 필요.
		((Child1)p2).printChild1(); // downCasting
		/*
		 * 대이연산자 기준 양쪽의 자료형이 다름에도 불구하고 오류가 나지 않는다.
		 * 이유는, 자식객체가 부모자료형으로 '자도형변환'되고 있기 때문.
		...
		System.out.println("반복문을 활용한 출력");
		// 반복문을 활용한 메서드 호출
		for(int i=0; i<arr.length; i++) {
			// 각 인덱스 별로 실제 참조하고 있는 자식클래스로 적절히 다운캐스팅 후, 메서드를
			// 호출하려면?
			// 1. instanceof 자료형 연산자 활용.
			// - 객체가 실제로 참조하고잇는 클래스를 확인하여 자료형과 같다면 true/ 다르다면 false를 반환
...
//			}
			 ...
			 * 	 실제 Runtime단계에서는 자시객체 내부의 오버라이딩된 메서드를 찾아가서 실행하는 기능.
			 */
			arr[i].print();
		}
	}
